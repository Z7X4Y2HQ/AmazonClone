...
export default HomeProductlist = ({ JSONdata, navigation }) => {
  ...
  return (
    <FlatList data={JSONdata} renderItem={({ item }) => (
      <View>
        <Pressable onPress={() => { navigation.navigate('ProductPage', item); setCurrentScreen("ProductPageFromHome") }}>
          <View style={styles.productContainer}>
            <Image style={styles.productImage} source={{ uri: item.images[0] }} />
            <View style={styles.productInfoContainer}>
              <Text numberOfLines={3} style={styles.productTitle} >{item.description}</Text>
