              ...
              <View style={{ flex: 1 }}>
                <View style={{ alignItems: "center", flex: 0.8, borderBottomWidth: 1, borderBottomColor: "#f2f2f2" }}>
                  ...
                  <View style={{ alignItems: "center", marginTop: 30, width: "100%" }}>
                    <Pressable onPress={() => navigation.navigate('LoginScreen')} style={{ backgroundColor: "#fdd031", borderWidth: 0.5, borderColor: '#e5e7e8', elevation: 2, borderRadius: 8, padding: 14, color: "black", width: "95%", alignItems: "center", marginBottom: 15 }}>
                      <Text>Sign in to your account</Text>
                    </Pressable>
                    <Pressable onPress={() => navigation.navigate('LoginScreen')} style={{ backgroundColor: "#fff", borderWidth: 0.5, borderColor: '#e5e7e8', borderRadius: 8, elevation: 2, padding: 14, color: "black", width: "95%", alignItems: "center", marginBottom: 15 }}>
                      <Text>Sign up now</Text>
                    </Pressable>
                  </View>
                </View>
                <View style={{ flex: 0.3, borderTopWidth: 1, borderTopColor: "#f2f2f2", alignItems: "center", marginTop: 14 }}>
                  <Pressable onPress={() => navigation.navigate('Home')} style={{ borderColor: '#e5e7e8', backgroundColor: "#fdd031", borderRadius: 8, elevation: 2, padding: 14, color: "black", width: "95%", alignItems: "center", marginTop: 30, marginBottom: 35 }}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ paddingHorizontal: 14, color: 'black' }}>
                  <View style={{ paddingVertical: 12, flexDirection: 'row' }}>
                    ...
                    <Text style={{ fontWeight: 'bold', fontSize: 20 }}>{addedToCart.newPrice}</Text>
                    <Text style={{ fontWeight: 'bold', lineHeight: 21, fontSize: 12 }}>{addedToCart.cents}</Text>
                  </View>
                </Text>
                ...
                <View style={{ paddingVertical: 12, paddingHorizontal: 14, borderBottomWidth: 0.8, borderColor: '#eaeaea', flexDirection: 'row' }}>
                  <Checkbox style={{ borderColor: "#9a9e9e", borderWidth: 1 }} value={isChecked} onValueChange={() => { setChecked(!isChecked); }} color={isChecked ? "#007186" : undefined} />
                  <Text>  Send as a gift. Easy Returns</Text>
                </View>
                <View style={styles.productContainer}>
                  <Image style={styles.productImage} source={{ uri: addedToCart.images[0] }} />
                  <View style={styles.productInfoContainer}>
                    <Text numberOfLines={2} style={styles.productTitle} >{addedToCart.description}</Text>
                    ...
                    <View style={{ paddingLeft: 5, flexDirection: 'column' }}>
                      ...
                      <Text style={{ fontSize: 14, color: 'black' }}><Text style={{ fontWeight: 'bold' }} >Color:</Text>  Harmony Blue/Chiffon Pink</Text>
                    </View>
                  </View>
                </View>
                  ...
                  <View style={{ paddingLeft: 14, padding: 5 }}>
                    <View style={{ flexDirection: 'row' }}>
                      <LinearGradient style={{ borderTopLeftRadius: 3, borderBottomLeftRadius: 3 }} start={{ x: 0, y: 1 }} end={{ x: 0, y: 0 }} colors={['#e8eaee', '#eff1f3', '#f6f9fe']}>
                        <Pressable onPress={() => { setCartItemsNum(cartItemsNum => cartItemsNum = cartItemsNum - 1) }} style={{ borderWidth: 0.8, borderColor: 'gray', paddingVertical: 5.2, paddingHorizontal: 11, borderTopLeftRadius: 3, borderBottomLeftRadius: 3 }}>
                          {cartItemsNum == 1 && <Feather name="trash" size={18} color="black" />}
                          {cartItemsNum > 1 && <Entypo name="minus" size={17.5} color="black" />}
                        </Pressable>
                      </LinearGradient>
                      <View style={{ borderWidth: 0.8, borderColor: 'gray', borderLeftWidth: 0, borderRightWidth: 0, paddingVertical: 3, paddingHorizontal: 20 }}>
                        <Text style={{ fontSize: 15, color: '#21697a' }}>{cartItemsNum}</Text>
                      </View>
                      <LinearGradient style={{ borderTopRightRadius: 3, borderBottomRightRadius: 3 }} start={{ x: 0, y: 1 }} end={{ x: 0, y: 0 }} colors={['#e8eaee', '#eff1f3', '#f6f9fe']}>
                        <Pressable onPress={() => { setCartItemsNum(cartItemsNum => cartItemsNum = cartItemsNum + 1) }} style={{ borderWidth: 0.8, borderColor: 'gray', paddingVertical: 4, paddingHorizontal: 8, borderTopRightRadius: 3, borderBottomRightRadius: 3 }}>
